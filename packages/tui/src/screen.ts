 ...
 * now — so it is an ordinary repaint unless the height also changes. Height
 * changes rebuild too, because the footer is clipped to the viewport.
 *
...
export class Screen {
  ...
  private paint(): void {
    if (this.interactive && this.footer.length > 0 &&
        (this.columns() < this.paintedWidth || this.rowCount() !== this.paintedHeight)) {
      // A resize can reflow or clip rows, invalidating their old positions.
      this.repaintAll();
      return;
    }
    this.clearFooter();
  ...
  private repaintAll(): void {
    ...
    // Completion can arrive before the resize debounce. Commit every newly
    // settled line, including the prefix previously clipped from the live view.
