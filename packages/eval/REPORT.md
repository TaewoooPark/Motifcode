...
This report was generated by `make_report.py` from the campaign's raw rows (`results/*.jsonl`), per-row logs, manifests and `results/campaign.log`, which live outside this repository (`~/motif-bench/` on the machine that ran the campaign). The suite builder, runner, grader and statistics it used are this package (`packages/eval`); the adapters that drove Codex and OpenCode through the same runner are described in Sections 3, 4 and 10.
...
Layout under `~/motif-bench/`: `env.sh` (toolchain pins, fixed git dates), `adapters/` (the three agent adapters + `common.sh`), `homes/` (pristine Codex/OpenCode configs), `manifests/`, `chunks/` (the stratified plan and `excluded.json`), `results/<harness>/*.jsonl` (rows), `logs/<harness>/…` (journal, agent log, patch, meta per row), `compare.py`, `make_report.py`. To repeat: build the suite with `motif-suite build`, verify with `motif-suite verify`, run chunks with `run_chunk2.sh`, compare with `compare.py`.
